  ...
  return (
    <article>
      ...
      <section className="mb-8" aria-labelledby="proactive-heading">
        ...
        <div className="space-y-4">
          <div className="card-base p-5">
            ...
            <a href="mailto:DCCI@elderaffairs.org" className="text-sm font-medium text-lavender-600 hover:text-lavender-700 transition-colors">Request a kit: DCCI@elderaffairs.org</a>
          </div>
          <div className="card-base p-5">
            ...
            <a href="https://www.fdle.state.fl.us" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-sm font-medium text-lavender-600 hover:text-lavender-700 transition-colors">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {['Install door alarms or chimes on exterior doors', 'Place locks high or low — out of typical eye level', 'Use door camouflage (paint doors to blend with walls)', 'Place a large STOP sign on exit doors', 'Enroll in a medical ID bracelet program', 'Keep recent photos updated — including what they typically wear', 'Inform neighbors to call you if they see the person outside alone', 'Reduce triggers: restlessness, searching for something, anxiety'].map((tip) => (
            <div key={tip} className="flex items-start gap-2.5 p-3 rounded-lg bg-white border border-warmgray-100 text-sm text-warmgray-700">
              <div className="w-1.5 h-1.5 rounded-full bg-lavender-400 mt-1.5 shrink-0" aria-hidden />
              {tip}
            </div>
          ))}
        </div>
      ...
      <div className="p-5 rounded-2xl bg-lavender-50 border border-lavender-200">
        <p className="text-sm font-semibold text-lavender-900 mb-2">Free wandering video</p>
        <p className="text-sm text-lavender-700 mb-2">The Alzheimer's Project video library includes a dedicated "Wandering" video with practical guidance. FSU REACH also hosts crisis-intervention videos on dementia emergencies.</p>
        <a href="https://alzheimersproject.org" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-sm font-medium text-lavender-700 hover:text-lavender-900 transition-colors">
